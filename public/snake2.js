...
var jablko;
var wynik = 0;
var gameover = false; //gdy jest rowny true - koniec gry
var socket;
var klawisz;
var i= 0;
var napisy = [];
...
document.getElementById('connect').addEventListener('click', ()=> {
    ...
    socket.addEventListener('open', () => {
        ...
        socket.addEventListener('message', (wia) => { //Obsługa danych przesyłanych przez serwer
            let wiad = JSON.parse(wia.data);
            if(wiad.typ === "plansza") //Wiadomośc standardowa, czyli przesyłanie klatki gry
            {
                if(first){
                    restart_game();
                    first = false
            ...
            else if(wiad.typ === "gameover") //Wiadomośc specjalna, informacja o przegranej
            {
                gameover = true;
                wynik = wiad.wynik;
            }
...
sendMsg.addEventListener('click', ()=> {
    wiadomosc = document.getElementById('nickname').value + ": " + document.getElementById('msg').value;
    socket.send(JSON.stringify({
        wiadomosc: wiadomosc,
    }));
})
...
function loop()
{
    ...
    window.addEventListener('keydown', (e) => { //Obłsuga klawiszy
    ...
    plansza.forEach(function(kwadrat) //Rysowanie całej gry: wszystko składa sie z róznokolorowych kwadratów
    {
